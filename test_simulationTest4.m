...
%                     end
            ...
            if doFDR1test
                ...
                tnum1 = sum( (sig_feature(:,3) >= 1) ); 
    %             tnum2 = sum( (sig_feature(:,3) >= 31) );
    ...
    %             resultFDR4(uu,1) = tnum4;
                if tnum1 > 0 
                    ...
                    tpVars = sig_feature(sig_feature(:,3) >= 1, 3);
    ...
    %             resultFDRlogistic4(uu,1) = tnum4;
                ...
                fprintf('FDRlogistic: %d %.3f\n', tnum1, resultFDRlogistic4est(uu).val(1));
            end
            ...
            if doPCLS2test
                ...
                if doFilterOutAnalysis
                    ...
                    if length(idx_filteredOut_union) > 0
                        ...
                        resultOPCM_filtered(uu).tnum1 = sum( (idx_filteredOut_union >= 1) );
                    else
                ...
                tnum1 = sum( (idx_union >= 1) );
    %             tnum2 = sum( (idx_union >= 31) );
    ...
    %             resultOPCM4(uu,1) = tnum4;
            ...
            if doSTOCSY2test
                ...
                tnum1 = sum( (idSTOCSYfound >= 1) );
    %             tnum2 = sum( (idSTOCSYfound >= 31) );
    ...
    %             resultSTOCSY4(uu,1) = tnum4;
                if tnum1 > 0
                    %checking out the detected variables in the 4th group
                    tpVars = idSTOCSYfound (idSTOCSYfound >= 1);
                    [accuracy pvalue1 pvalue2] = computeLogisticRegressionPerformance(data(:,tpVars),label_data);
                ...
                fprintf('STOC: %d %.3f\n', tnum1, resultSTOCSY4est(uu).val(1));
            end
        end
    ...
    tpcmd = sprintf('save(''allres_zerolayers_plus_randomlayer_withFilteredOutVars_%.2f.mat'',''allres'',''alllevels'', ''noiselevel'', ''TotITER'');', noiselevel);
    eval(tpcmd);
