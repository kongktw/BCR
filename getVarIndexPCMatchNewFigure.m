function [idx_inside, Angles, MDistSq, cutoff_angle, cutoff_dist, outlier_id, sampleInsidePval, curLbl]=getVarIndexPCMatchNewFigure(Params)
...
%여기부터 603까지가 OPLS 그림 그리는 것인데 주석처리하면 안그리게 됨(반복 테스트 할 때)
...
%     plot(score_data(idLB2,idPC1), score_data(idLB2,idPC2), 'rx', 'MarkerSize', msize);
    ...
    score_data = (1/scale).*score_data; %스케일 조정(단위에 주의)
...
%         legend( num2str( (1:length(highIDmany))' ) ); %score data와 합치지 않을
%         때 범례를 표시해줄 때 사용
    end
...
ylabel('Predictive component 1')
title('')
legend('Label 0', 'Label 1', 'Location', 'East')
axis([-0.0003 0.0003 -0.0003 0.0003]);
  
end



% 340부터 여기까지 OPLS 그림 그리는 것인데 주석처리하면 안그리게 됨(반복 테스트 할 때)
...
%             end
        ...
        legend( num2str( (1:length(highIDmany))' ) ); %score data와 합치지 않을때 범례를 표시해줄 때 사용
...
xlabel('Orthogonal component 1')
ylabel('Predictive component 1')
title('loading with variables found')
legend('Layer 1', 'Layer 2', 'Layer 3')
axis([-0.00007 0.00007 -0.00007 0.00007]); 

  
end

if drawfig && (length(idPCAll) == 3) && size(X,1) > 1 %label이 3개 일 때
